...
byte send_page[32] = "Graham is a genius! 32 bytes....";      // data to write in page mode (max. 32 bytes per page)
byte send_user[100]="This is a test of writing more than 32 bytes to a user defined page size"; // data to write in user defined page test
byte read_seql[80], read_page[32], read_byte, read_user[100]; // to hold data returned from functions
...
void setup() {
  ...
  for (addr = 0; addr < 32767; addr++){   // loop through entire chip - max for 'short' variable
    RAM_RITEByte(addr, dummy);}           // write 'blanking' data = dummy variable
  RAM_RITEByte(32767, dummy);             // blank last address
  Serial.print("...done");
  ...
  for (addr = 0; addr < 5; addr++){       // Write 5 individual bytes to memory eg 0 to 4
    RAM_RITEByte(addr, dataS);            // write a byte of data to that address
  ...
  ReadStatus();                           // call function to confirm mode
  int startaddr=30;                       // set start address eg 30
  for (j=0; j<5; j++){                    // loop through 5 page addresses
    addr=startaddr+(j*32);                // each page address MUST be 32 bytes after previous address
  ...
  RAM_RITESeq(addr, send_seql, sizeof(send_seql)); // Write to memory starting at address set using the array of characters defined above
  ...
  WriteStatus(MODE_SEQL);                             // set to send/receive multiple bytes of data
  ReadStatus();                                       // call function to confirm mode
  startaddr = 0x101;                                  // set a start address eg 0x101
  for (j=0; j<5; j++){                                // loop through 5 user defined page addresses
    addr=startaddr+(j*pagelength);                    // each page address MUST be 'pagelength' bytes after previous address
...
void RAM_RITEByte(short addr, byte data_byte){
  digitalWrite(CS, LOW);                    // open comms with device
...
void RAM_RITESeq(short addr, byte *send_seql, int dsize){
  digitalWrite(CS, LOW);                    // open comms with device
  SPI.transfer(RAM_RITE);                   // send write command
  SPI.transfer16(addr);                     // send address
  for (i = 0; i < dsize; i++) {
